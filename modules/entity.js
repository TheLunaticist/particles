...
import { Rect } from "/m/rectangle.js";
import { Game } from "/m/game.js";
import { AssetManager } from "/m/assetManager.js";
import {
    CalledVirtualFunctionError,
    instanceAssert,
    typeAssert,
} from "/m/debug.js";
import { canvas, ctx, drawImageRelative } from "/m/graphics.js";
...
class Tower extends Entity {
    ...
    static canPlace(x, y, type) {
        ...
        //HQ
        if (
            Vec2.doVectorSquaresIntersect(
                posBlue,
                sizeBlue,
                Game.state.hq.pos,
                Game.state.hq.type.size,
            )
        ) {
            return false;
        }
    ...
    draw() {
        if (this.type == TowerType.SNIPER) {
            ...
            //drawing base
            ctx.drawImage(
                AssetManager.textures["sniper_turret_base"],
                this.rect.left,
                this.rect.top,
            );
            ...
            if (this.target != undefined) {
                const targetVector = Vec2.subtract(
                    this.target.rect.getCenter(),
                    this.rect.getCenter(),
                );
                angle =
            ...
            ctx.rotate(angle);
            ctx.drawImage(
                AssetManager.textures["sniper_turret_head"],
                -HEAD_SIZE / 2,
                -HEAD_SIZE / 2,
            );
            ...
            ctx.drawImage(
                AssetManager.textures["mg_turret_base"],
                this.rect.left,
                this.rect.top,
            );
            ...
            ctx.rotate(angle);
            ctx.drawImage(
                AssetManager.textures["mg_turret_head"],
                -48 / 2,
                -48 / 2,
            );
            ...
            //base
            ctx.drawImage(
                AssetManager.textures["rocket_turret_base"],
                this.rect.left,
                this.rect.top,
            );
            //head
            ...
            let angle;
            if (this.target != undefined) {
                let targetVector = Vec2.subtract(
                    this.target.rect.getCenter(),
                    this.rect.getCenter(),
                );
                angle =
            ...
            ctx.rotate(angle);
            ctx.drawImage(
                AssetManager.textures["rocket_turret_head"],
                -32 / 2,
                -32 / 2,
            );
    ...
    acquireTarget() {
        if (Game.state.enemies[0] != undefined) {
            ...
            for (let i = 0; i < Game.state.enemies.length; i++) {
                vecToEnemy = Vec2.subtract(
                    Game.state.enemies[i].rect.getCenter(),
                    this.rect.getCenter(),
                );
...
export class Projectile extends Entity {
    ...
    draw() {
        if (this.type == ProjectileType.BALL) {
            ctx.drawImage(
                AssetManager.textures["cannon_ball"],
                this.rect.left,
                this.rect.top,
            );
        } else if (this.type == ProjectileType.ROCKET) {
            let angle =
                Math.atan2(this.vel.y, this.vel.x) + 2 * Math.PI * (3 / 4);
            let center = this.rect.getCenter();
