...
export class AssetManager {
    ...
    static async load() {
        ...
        AssetManager.loadFont(
            "Orbitron",
            "url(./assets/fonts/Orbitron-Regular.ttf)",
            promises,
        );
        ...
        if (rejectReasons.length !== 0) {
            return Promise.reject(
                new LoadAssetError(rejectReasons, AssetManager.textureNumber),
            );
        }
    }
...
export class LoadAssetError extends LoggableError {
    ...
    log() {
        console.error(
            this.name + ":",
            this.message +
                ` [${this.totalAssetCount - this.rejectReasons.length}/${this.totalAssetCount}]\nerror events:`,
            this.rejectReasons,
        );
    }
}
